...
expected_manifest_real=$(realpath -e -- "$expected_manifest")
[[ $(dirname "$manifest") == "$bundle_dir" && $(dirname "$checksums") == "$bundle_dir" ]] || { echo "bundle inputs must share a directory" >&2; exit 1; }
case "$expected_manifest_real" in
    "$bundle_dir_real"/*) echo "trusted expected manifest must be outside the bundle directory" >&2; exit 1 ;;
esac
...
python3 - "$checksums" "$archive" "$manifest" <<'PY'
import hashlib, pathlib, re, sys
checksums_path, archive_path, manifest_path=map(pathlib.Path, sys.argv[1:])
lines=checksums_path.read_text(encoding='utf-8').splitlines()
expected={'am2-host-security.tar.gz':archive_path,'host-security-manifest.json':manifest_path}
seen=set()
for line in lines:
    ...
    name=match.group(2)
    if hashlib.file_digest(expected[name].open('rb'),'sha256').hexdigest() != match.group(1):
        raise SystemExit(f'host-security checksum mismatch: {name}')
    seen.add(name)
