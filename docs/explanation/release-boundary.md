...
The source contract and deterministic bundle/verifier are implemented. Installation, activation, receipt, and drift-audit work are not implemented yet. Existing host files may protect live traffic correctly without proving that their installed bytes came from the current source contract; behavior health and provenance are separate gates.

Cloudflare real-IP data is externally refreshed rather than fully Git-static. Its fetched ranges require a separate timestamped source record, validated generated bytes, protected receipt, stale-data policy, rollback, and refresh audit.
